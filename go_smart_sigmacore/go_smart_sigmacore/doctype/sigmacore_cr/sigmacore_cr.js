...
frappe.ui.form.on("SigmaCore CR", {
	onload(frm) {
        //filter level 1 
        frm.set_query('module_name', 'scopes', function(doc, cdt, cdn) {
        ...
        //filter level 2
        frm.set_query('section', 'scopes', function(doc, cdt, cdn) {
            let row = locals[cdt][cdn];
        ...
        //filter level 3
        frm.set_query('target', 'scopes', function(doc, cdt, cdn) {
            let row = locals[cdt][cdn];
...
function childTableAccessCondations(frm) {
    // map each row
        frm.fields_dict.scopes.grid.grid_rows.map(row => {
